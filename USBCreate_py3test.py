...
def wait_for_input():
	wait_for_input = str(input('\n\nHit ENTER once you have finished reading to continue: '))
wait_for_input()
# implement this for custom modules to use exp
def exp_test():
    if exp == 1:
        print('############################### USBCreate ################################') 
        exp_test_input = str(input('This configuration is EXPERIMENTAL. Are you sure you wish to continue (Y/N)? '))
